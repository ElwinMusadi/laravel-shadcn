...
<aside
    ...
    @if (! $mobile) x-cloak x-show="sidebarExpanded" x-transition.opacity x-bind:data-state="sidebarExpanded ? 'open' : 'closed'" @endif
    {{ $attributes->class([
        'flex shrink-0 flex-col border-sidebar-border bg-sidebar text-sidebar-foreground',
        'hidden min-h-[calc(100svh-1rem)] w-[var(--app-sidebar-expanded)] border lg:flex lg:rounded-xl' => ! $mobile,
