  ...
  @foreach ($groups as $group)
    <section @class(['mt-auto' => ($group['position'] ?? null) === 'bottom']) aria-labelledby="sidebar-group-{{ $mobile ? 'mobile' : 'desktop' }}-{{ $group['key'] ?? $loop->index }}">
      @if (filled($group['label'] ?? null))
        <p id="sidebar-group-{{ $mobile ? 'mobile' : 'desktop' }}-{{ $group['key'] ?? $loop->index }}" class="mb-2 px-2 text-xs font-medium text-sidebar-foreground/70">
          {{ $group['label'] }}
        </p>
      @endif
      ...
      <ul class="flex flex-col gap-1" role="list">
        @if ($group['quickActions'] ?? false)
          <li class="flex items-center gap-2 pb-2">
            <x-ui.button size="sm" class="h-8 min-w-0 flex-1 justify-start text-xs" data-test="sidebar-quick-create">
              <x-ui.icon name="circle-plus" class="size-4" />
              {{ __('Quick Create') }}
            </x-ui.button>

            <x-ui.button variant="outline" size="icon" class="size-8 shrink-0" aria-label="{{ __('Inbox') }}" data-test="sidebar-inbox">
        ...
        @foreach ($group['items'] ?? [] as $item)
          ...
          <li>
            <a href="{{ $href }}" @if ($item['wireNavigate'] ?? isset($item['route'])) wire:navigate @endif @if ($isActive) aria-current="page" @endif data-test="sidebar-navigation-item-{{ $item['key'] ?? $loop->index }}" @class([
                'flex h-8 items-center gap-2 rounded-md px-2 text-sm font-medium outline-none transition-colors focus-visible:ring-2 focus-visible:ring-sidebar-ring focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar',
