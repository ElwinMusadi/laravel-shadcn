...
<nav
    {{ $attributes->class('flex h-full w-full flex-col')->merge(['aria-label' => $label ?? __('Primary navigation')]) }}
    data-test="application-navigation-{{ $mobile ? 'mobile' : 'desktop' }}"
>
        ...
        <section @class(['mt-auto' => ($group['position'] ?? null) === 'bottom']) aria-labelledby="sidebar-group-{{ $mobile ? 'mobile' : 'desktop' }}-{{ $group['key'] ?? $loop->index }}">
            @if (filled($group['label'] ?? null))
                <p
                    ...
                    class="px-2 pb-1 text-xs font-medium text-sidebar-foreground/70"
                >
                    {{ $group['label'] }}
                </p>
            @endif
            ...
            <ul class="flex flex-col gap-1" role="list">
                @if ($group['quickActions'] ?? false)
                    <li class="flex items-center gap-2 pb-2">
                        <x-ui.button class="min-w-0 flex-1 justify-start" data-test="sidebar-quick-create">
                            <x-ui.icon name="circle-plus" class="size-4" />
                            {{ __('Quick Create') }}
                        </x-ui.button>

                        <x-ui.button variant="outline" size="icon" aria-label="{{ __('Inbox') }}" data-test="sidebar-inbox">
                ...
                @foreach ($group['items'] ?? [] as $item)
                    ...
                    <li>
                        <a
                            ...
                            @class([
                                'flex min-h-8 items-center gap-2 rounded-md px-2 text-sm font-medium outline-none transition-colors focus-visible:ring-2 focus-visible:ring-sidebar-ring focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar',
