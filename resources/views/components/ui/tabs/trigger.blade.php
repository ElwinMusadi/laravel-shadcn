...
<button
    ...
    @keydown.right.prevent="const tabs = Array.from($el.closest('[role=tablist]').querySelectorAll('[role=tab]:not([disabled])')); tabs[(tabs.indexOf($el) + 1) % tabs.length].focus()"
    @keydown.left.prevent="const tabs = Array.from($el.closest('[role=tablist]').querySelectorAll('[role=tab]:not([disabled])')); tabs[(tabs.indexOf($el) - 1 + tabs.length) % tabs.length].focus()"
    @keydown.home.prevent="$el.closest('[role=tablist]').querySelector('[role=tab]:not([disabled])').focus()"
    @keydown.end.prevent="Array.from($el.closest('[role=tablist]').querySelectorAll('[role=tab]:not([disabled])')).at(-1).focus()"
